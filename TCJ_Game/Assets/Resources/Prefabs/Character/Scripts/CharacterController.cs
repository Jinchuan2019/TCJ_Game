...
public class CharacterController : MonoBehaviour, ISaveable
{
    ...
    public Vector3 NextPos { get => _nextPos; set => _nextPos = value; }
    ...
    public SaveData Save()
    {
        ...
        saveData.position = NextPos;
        saveData.key = this.key;
        saveData.isOpenTheDoor = false;
        saveData.moveable = isPlayer;
        return saveData;
    }
    public void LoadSave(SaveCharacter save)
    {
        this.transform.position = save.position;
        this.key = save.key;
        this.isOpen = save.isOpenTheDoor;
    }
}
